...
import {db} from "../firebase"; //The database we created inside the firestore
import {doc, serverTimestamp, setDoc} from
"firebase/firestore";
import {useNavigate} from "react-router-dom"; //To allow user to navigate to the home page
...
export default function SignUp() {
  ...
  async function onSubmit(e){
    ...
    try {
      ...
      delete formData.password //delete the password save as its more secure to use Firebase password storage because you dont want to save the password on the database
      formDataCopy.timestamp = serverTimestamp //To show the time the user is registered,using a method that captures the time the server is accessed
      ();

      await setDoc(doc(db, "users", user.uid), formDataCopy); //"users" is what I'll see under Cloud FireStore "start collection. user uid on our database is accessed using user.uid"
      //toast.success("Sign Up was successful!");
      navigate("/"); //Navigate to home page after adding the china onto db
    } catch (error) {
      toast.error("Something Went Wrong with the registration");
    } //I should use an if statement for if both invalid and invalid messaging...but not neccessary if sign up was successful
  }
  return (
    <section>
